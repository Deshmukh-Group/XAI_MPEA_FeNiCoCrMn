...
for file in tqdm(files):
    data = pd.read_csv(path + "/" + file, sep='\s+', header=None, skiprows=18)
    data.columns = ['id', 'type', 'x', 'y', 'z', 'positive', 'negative', 'diff']

    # get the data with id from 1401 to 2200
    data = data[(data['id'] >= 1601) & (data['id'] <= 2000)]
...
for label, data in datasets.items():
    sns.histplot(data, bins=1000, fill=True, kde=True, label=label, legend=True)
...
plt.savefig('data3_middle_2layer.png', dpi=300)
# plt.show()