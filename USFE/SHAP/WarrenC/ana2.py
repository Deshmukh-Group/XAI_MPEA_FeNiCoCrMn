...
merged_df = merged_df.nsmallest(100, 'Y_Actual')


# get min and max of Y_Actual
print(merged_df['Y_Actual'].min(), merged_df['Y_Actual'].max())
...
for col in interaction_groups:
    for interaction in col:
        count_high = (merged_df[interaction] > 0.18).sum()
        count_low = (merged_df[interaction] < -0.18).sum()
        print(f'{interaction}: >0.2 count = {count_high}, <-0.2 count = {count_low}')
...
cbar.set_label('USFE Calculated', fontsize=10)

# Adjust layout
# plt.tight_layout()
# plt.savefig('5x5_panel_plot.png', dpi=300)
plt.show()
